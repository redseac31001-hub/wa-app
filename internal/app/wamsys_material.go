...
func deriveLocalWamsysBytes(input wamsysMaterialInput, label string, length int) []byte {
	seed := strings.Join([]string{
		"byte-v-forge-wa-wamsys-local/v1",
		label,
		phoneCC(input.Phone),
...
// The runtime keeps them behind a dedicated material provider so registration
// maps do not leak opaque blobs into generic phone profile fields. The default
// provider is a local APK-shape emulator: fresh fields are random per request,
// while stable fields are derived from the registration native state.
